...
- [x] 12.5 Scrape and convert all ~30 articles from WordPress to markdown with correct frontmatter (title, slug, categories, excerpt, featuredImage, publishDate) — scraped all 37 articles; upgraded schema from single `category` to `categories` array with 10 category values (added Parenting, Relationships, Screen Time, Personal Development); downloaded 27 featured images (10 returned 404 from WordPress CDN, used placeholder); updated ArticleCard, articles index, article detail page, and RSS feed for multi-category support; created `public/_redirects` with 37 Netlify 301 redirect rules for backward URL compatibility (original site served articles at root-level `/{slug}/`, new site at `/articles/{slug}/`)
- [x] 12.5a Re-scrape all 37 articles for embedded videos (YouTube/Vimeo iframes) missed during initial scrape — wrote `scripts/detect-article-embeds.mjs`; found 4 articles with YouTube iframes stripped by the HTML-to-markdown converter: `how-to-raise-a-happy-child` (qulOOjl_qcY), `teaching-your-kids-about-forgiveness-and-why-its-important` (0I-I7HZr5FM), `dealing-with-addiction-mental-illness-more-in-the-home` (hdIKwByV2UI), `surviving-and-thriving-parenting-tips-and-tricks-an-interview-with-amber-trueblood` (ZktaYaG8wRo); added responsive `<iframe>` embeds to each article's markdown
- [x] 12.5b Add two-column layout to article detail page (`src/pages/articles/[...slug].astro`) — left column: article content (featured image, date, prose body, back link); right sidebar: "About Us" fixed text, social media links (Facebook, YouTube, Instagram), "Recent Posts" (3 most recent articles with thumbnails); omitted "Follow Us" Facebook page embed (low value) and "Leave a Comment" (not wired, no comments facility planned); added purple hero banner matching original article header style
...
- [x] 12.10 Scrape actual text content from the WordPress **Projects** page (`thefamproject.com/projects/`) and replace fabricated copy in `src/pages/projects.astro` — scraped all 6 sections: Our Projects hero, FAM Fun Pass (sunburst bg), FAM Central, Vendor section, Chris Lange testimonial, FUNdraising form
